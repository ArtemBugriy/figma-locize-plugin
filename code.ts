...
// Non-empty sentinel meaning "explicitly unbound". Plugin data on instance sublayers
// inherits from the main component, and setting it to '' just reverts to that inherited
// value — so we mask it with a non-empty override that we treat as no key.
...
/** The node's bound locize key, or '' if unbound (empty or the "cleared" sentinel). */
function readNodeKey(node: BaseNode): string {
  const k = node.getPluginData(PLUGIN_KEY_KEY);
  return (!k || k === CLEARED_KEY) ? '' : k;
}
...
function generateKeys(textNodes: TextNode[], namespace: string, mergeSameName = false): ScanItem[] {
  ...
  for (const node of textNodes) {
    const existingKey = readNodeKey(node);
...
/** Scan the current selection and post a scan-result. Shared by scan-selection and clear-keys. */
async function postScanResult(namespace: string, ignoreHidden: boolean, mergeSameName: boolean): Promise<void> {
  const selection = figma.currentPage.selection;
  if (!selection.length) {
    figma.ui.postMessage({ type: 'scan-result', items: [], warning: 'No selection' });
    return;
  }
  const { nodes: textNodes, truncated } = collectTextNodes(selection as readonly SceneNode[], MAX_SCAN_NODES, ignoreHidden);
  if (!textNodes.length) {
    figma.ui.postMessage({ type: 'scan-result', items: [], warning: 'No text nodes found' });
    return;
  }
  const items = generateKeys(textNodes, namespace, mergeSameName);
  try {
    const selMap = await getSelectionMap();
...
figma.ui.onmessage = async (msg) => {
  switch (msg.type) {
    ...
    case 'scan-selection': {
      await postScanResult(msg.namespace || DEFAULT_NS, !!msg.ignoreHidden, !!msg.mergeSameName);
      break;
    }
    case 'clear-keys': {
      // Unbind selected nodes: remove the locize key (and origName), restoring the
      // original layer name if an old version had renamed it. Report what was cleared
      // so the UI can zero those rows in place (no full re-scan).
      const items: { nodeId: string }[] = msg.items || [];
      const cleared: { nodeId: string; name: string }[] = [];
      let failed = 0;
      await Promise.all(items.map(async ({ nodeId }) => {
        const node = await figma.getNodeByIdAsync(nodeId);
        if (node && node.type === 'TEXT') {
          const textNode = node as TextNode;
          const orig = textNode.getPluginData(PLUGIN_ORIG_NAME_KEY);
          if (orig && orig !== textNode.name) { try { textNode.name = orig; } catch (e) { console.error(e); } }
          try {
            // Non-empty sentinel — persists as an override on instance sublayers
            // (setting '' would just revert to the main component's inherited key).
            textNode.setPluginData(PLUGIN_KEY_KEY, CLEARED_KEY);
            textNode.setPluginData(PLUGIN_ORIG_NAME_KEY, '');
          } catch (e) { console.error(e); }
          // Verify the node now reads as unbound.
          if (readNodeKey(textNode)) { failed++; }
          else { cleared.push({ nodeId, name: textNode.name }); }
        }
      }));
      if (failed > 0) {
        figma.notify(`${failed} node(s) couldn't be unbound (inside a component instance?)`, { error: true });
      } else {
        figma.notify(`Cleared ${cleared.length} node(s)`);
      }
      figma.ui.postMessage({ type: 'keys-cleared', items: cleared });
      break;
    }
    case 'apply-keys': {
      const items: ScanItem[] = msg.items;
      const applied: { nodeId: string; name: string }[] = [];
      let renameFailed = 0;
      await Promise.all(items.map(async (item) => {
        ...
        if (node && node.type === 'TEXT') {
          ...
          // Write the namespace into the layer name so readers outside this plugin can
          // reconstruct the full key from the name alone (see README "Layer naming").
          // The name is always rebuilt from the key, never appended to, so re-applying
          // with a different namespace yields "submit (NewNs)", not "submit (A) (B)".
          const { namespace: ns, localKey } = parseKey(item.key);
          const newName = ns ? `${localKey} (${ns})` : localKey;
          // Layer names are read-only inside component instances, where this throws.
          try {
          ...
          // Report the name the node actually ended up with, so the table matches canvas.
          applied.push({ nodeId: item.nodeId, name: textNode.name });
        }
      }));
      figma.ui.postMessage({ type: 'keys-applied', items: applied, renameFailed });
      figma.ui.postMessage({ type: 'namespaces-result', namespaces: collectAssignedNamespaces() });
      if (renameFailed > 0) {
        figma.notify(`Keys applied, but ${renameFailed} layer(s) couldn't be renamed (inside a component instance?)`, { error: true });
      } else {
